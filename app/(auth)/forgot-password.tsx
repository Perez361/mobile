...
export default function ForgotPasswordScreen() {
  ...
  async function onSubmit(data: FormData) {
    setLoading(true)
    try {
      await createImporterClient().auth.resetPasswordForEmail(data.email, {
        redirectTo: 'importflowpro://reset-password',
      })
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        <View style={s.header}>
          <SafeAreaView edges={['top']}>
            ...
            <Text style={s.subtitle}>We'll send you a link to reset your password</Text>
          </SafeAreaView>
        </View>
        ...
        <View style={s.form}>
          {sent ? (
            <View style={s.successBox}>
              ...
              <Button onPress={() => router.replace('/(auth)/login')} style={{ marginTop: Spacing.md }}>Back to Sign In</Button>
            </View>
          ) : (
            <>
